...
for vegm, ax, t in zip(veg_masks, axes, titles):     
    ...
    ax.plot(time, annual_gosif, color = 'k', lw = 4.0, label = 'GOSIF-GPP')
    ax.fill_between(time, annual_upper, annual_lower, color = 'k', alpha = 0.3)
    ax.set_xticks(np.arange(2001,2017,3))
    ax.set_title(t)
    
for ax in (ax1, ax2, ax3, ax5):
    ax.set_xticklabels([])

ax1.set_ylabel('GPP [PgC yr-1]')
ax4.set_ylabel('GPP [PgC yr-1]')
